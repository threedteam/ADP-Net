...
function [TX_sd, TX_ed] = process_left_view(RX_u, RX_num, RX_M, RX_sharp_th, RX_l, RX_num_holes, RX_lb_bk_fg, RX_Wi)
...
while RX_lb_bk_fg == 0
    if i < RX_Wi  % 1-based
        if RX_M(i + 1) ~= - 1  % 非空洞点
            if RX_M(i - RX_num_holes) - RX_M(i + 1) >= RX_sharp_th % 空洞左边缘区域为前景像素点；(无论左视图还是右视图，前景像素的深度值大于背景像素的深度值)，只膨胀空洞左边缘。
                RX_lb_bk_fg = 1;
