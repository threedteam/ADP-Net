...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [tx_ides, tx_m] = threed_image_warping_old(rx_iref, rx_d, rx_min_z, rx_max_z, rx_pr, rx_ides_rt, rx_ps)
        ...
        case {'F'}
            ...
            for i = Rect(2) : Rect(4)  % 按行扫描
                for j = Rect(1) : Rect(3)
                    ...
                    [u, v] = projXYZtoUV(m_Height, rx_ps, x, y, z); % 基数为0
                    u = round(u);
                    v = round(v);
                    % 判断[u, v]是否落在I1内
                    InIs = inpolygon(u, v, IsXV, IsYV);  % 1 = True
                    if (InIs == 1)% 在才处理
                        tx_ides(v + 1, u + 1, :) =  rx_iref(i + 1, j + 1, :);
                        tx_m(v + 1, u + 1) = round(d); % 求解warped depth map时不需要坐标变换，直接拷贝即可。uint8转换成double
                    end
%                     if v+1==179 && u+1==690 % debug
%                         fprintf('i=%d, j=%d, d=%f\n', i+1, j+1, d); 
%                     end
                end
            end
