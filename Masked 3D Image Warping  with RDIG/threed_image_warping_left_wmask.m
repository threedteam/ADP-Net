...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [tx_ides, tx_m, bk_ides, bk_m, mask_ides] = threed_image_warping_left_wmask(mask, rx_iref, rx_d, rx_min_z, rx_max_z, rx_pr, rx_ides_rt, rx_ps)
...
bk_ides = uint8(zeros(m_Height, m_Width, Clr)); % 合成视图，uint8
bk_m(1 : m_Height, 1 : m_Width) = -1; % non-hole matrix, each element varies from -1 to 255. All elements are initialized to -1
mask_ides = uint8(zeros(m_Height, m_Width, Clr));
                ...
                for j = Rect(1) : Rect(3)
                    ...
                    if (InIs == 1)% 在才处理
                        if tx_m(v_round+1, u_round+1)<d+th_d
                            ...
                            %对bk_ides 和bk_m处理的片段
                            bk_ides(v_round+1, u_round+1, :) = rx_bk(i + 1, j + 1, :);
                            bk_m(v_round+1, u_round+1) = round(d);
                            %对前景mask进行投影
                            mask_ides(v_round+1, u_round+1, :) = mask(i + 1, j + 1, :);
                        ...
                        if tx_m(v+1, u+1)+th_d<d
                            ...
                            %对bk_ides 和bk_m处理的片段
                            bk_ides(v+1, u+1, :) = rx_bk(i + 1, j + 1, :);
                            bk_m(v+1, u+1) = round(d);
                            mask_ides(v+1, u+1, :) = mask(i + 1, j + 1, :);
                        end
                        if tx_m(v+1, u-1)+th_d<d
                            tx_ides(v+1, u-1, :) = rx_iref(i + 1, j + 1, :);
                            tx_m(v+1, u-1) = round(d);
...
%                         tx_m(v + 1, u + 1) = round(d); % 求解warped depth map时不需要坐标变换，直接拷贝即可。
    ...
    bk_m(mask_ides(:, :, 1) == 255) = -1;  %将bk_m中前景部分设为-1
%     toc
    %% show
%     figure(1);
%     imshow(tx_ides);
