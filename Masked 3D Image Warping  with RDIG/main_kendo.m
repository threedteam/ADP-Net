...
calib_params_kendo; % read the calibration parameters of ��kendo�� sequence
data_path = 'E:\Desktop\research\dibr\paper\nowtest\dataset\kendo\';
mask_folder_root_path = 'E:\Desktop\research\dibr\paper\nowtest\dataset\kendo\instance\out_kendo5\';

save_path = 'E:\Desktop\research\dibr\paper\nowtest\dataset\kendo\dibr';
...
for idx = 0 : 99
    ...
    if (idx < 10)
        FileName = strcat(data_path, 'images\kendo5\color-cam5-f00', int2str(idx), '.png');
        IR = imread(FileName);
        
        % ��ȡIR��Ӧ�����ͼ
        FileName = strcat(data_path, 'depth\kendo5\depth-cam5-f00', int2str(idx), '.png');
        D = imread(FileName);
        
        % ��ȡ��Ӧ��instanceͼ
        frame_name = strcat('color-cam5-f00', int2str(idx));
    else
        FileName = strcat(data_path, 'images\kendo5\color-cam5-f0', int2str(idx), '.png');
        IR = imread(FileName);
        
        % ��ȡIR��Ӧ�����ͼ
        FileName = strcat(data_path, 'depth\kendo5\depth-cam5-f0', int2str(idx), '.png');
        D = imread(FileName);
...
%     % ��ȡ�����3�����ͼ��
%     FileName = strcat(data_path, 'images\kendo5\color-cam5-f', int2str(idx), '.png');
%     IR = imread(FileName);
%     
%     % ��ȡIR��Ӧ�����ͼ
%     FileName = strcat(data_path, 'depth\kendo5\depth-cam5-f', int2str(idx), '.png');
%     D = imread(FileName);
%     
%     % ��ȡ��Ӧ��instanceͼ
...
%     [Ides, M] = threed_image_warping_left(IR, D, MinZ, MaxZ, m1_ProjMatrix, m5_RT, m5_ProjMatrix);
    ...
    [Ides, M, Ides_bk, M_bk, fg_map] = threed_image_warping_left_wmask(instance_map, IR, D, MinZ, MaxZ,  m1_ProjMatrix, m5_RT, m5_ProjMatrix);

    
%     [Ides, M] = threed_image_warping(IR, D, MinZ, MaxZ, m1_ProjMatrix, m5_RT, m5_ProjMatrix); % Ides: generated destination image, uint8; M: generated non-hole matrix, double
...
%     [Ides_map, M_map] = big_hole_dilation(Ides_map, M_map, th_big_hole, sharp_th, n_dilation, rend_order, wi, hi);
    ...
    %% ����Ŀ��ͼ�������Ϣ
     D = M;
          ...
          %���������Ϣ���ɲ���
     %FileName_D = strcat('G:\thesis_experiment_rxw\datasets\kendo\improved_dibr\D_kendo\Is_kendo_D', int2str(idx), '.txt');
...
% --- �����ɫ�ն���Ŀ��ͼ����mask ---
% ���岢����Ŀ���ļ���
save_folder_Ides = fullfile(save_path, 'Ides_Kendo');
...
% ���岢����Mask�ļ���
save_folder_M = fullfile(save_path, 'M_Kendo');
...
% --- �����ɫ�ն���Ŀ�걳��ͼ����mask ---
% ���岢����Ŀ�걳��ͼ�ļ���
save_folder_Ides_bk = fullfile(save_path, 'Ides_Kendo_bk');
...
% ���岢����Ŀ�걳��ͼMask�ļ���
save_folder_M_bk = fullfile(save_path, 'M_Kendo_bk');
...
%     %�����ɫ�ն���Ŀ��ͼ����mask
%     FileName_Ides = strcat(save_path, '\Ides_kendo\Is_kendo_des', int2str(idx), '.png');
%     imwrite(Ides, FileName_Ides);
%     FileName_M = strcat(save_path, '\M_kendo\Is_kendo_des', int2str(idx), '_mask.png');
%     imwrite(M, FileName_M);
% %     %�����ɫ�ն���Ŀ�걳��ͼ����mask
%     FileName_Ides = strcat(save_path, '\Ides_kendo_bk\Is_kendo_bk_des', int2str(idx), '.png');
%     imwrite(Ides_bk, FileName_Ides);
%     FileName_M_bk = strcat(save_path, '\M_kendo_bk\Is_kendo_bk_des', int2str(idx), '_mask.png');
%     imwrite(M_bk, FileName_M_bk);
%     %����ǰ������mask
%     FileName_fg_map = strcat(save_path, '\fg_instance\Is_kendo_des', int2str(idx), '_mask.png');
%     imwrite(fg_map, FileName_fg_map);
    
end
% save('ballet_hole_pers.mat','hole_pers');
save('kendo_warp_time.mat','warp_time');
